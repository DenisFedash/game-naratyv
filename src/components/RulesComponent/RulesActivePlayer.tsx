...
export const RulesActivePlayer: FC<RulesComponentProps> = ({ textTr, lang, role }) => {

const searchParams = new URLSearchParams(window.location.search);
const paramValue = searchParams.get("sessionIdentificator");
  console.log("paramValue:", paramValue);

    const [rulesByRole, setRulesByRole] = useState<any[]>([]);

   useEffect(() => {
     const fetchRules = async () => {
       const rules = await getAllRules();
       const rulesByRole: any[] = rules.filter((rule: any) => rule.role === role);
       setRulesByRole(rulesByRole);
     };
   ...
   useEffect(() => {
     if (paramValue) {
       console.log('sessionIdentificator:', paramValue);
      addPlayersInGame(paramValue);
     }
   }, [paramValue]);
